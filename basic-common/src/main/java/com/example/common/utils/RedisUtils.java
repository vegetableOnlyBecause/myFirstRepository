...
public class RedisUtils {
    ...
    public void setRedisOperator(RedisOperator redisOperator) {
        RedisUtils.redisOperator = redisOperator;
    }
    ...
    public static <T> T get(String key, Class<T> clazz, long time,
        ...
        obj = supplier.get();
        redisOperator.set(key, JSON.toJSONString(obj), time);
        return obj;
    }
    ...
    public static <T> List<T> list(String key, Class<T> clazz, long time,
        ...
        list = supplier.get();
        redisOperator.set(key, JSON.toJSONString(list), time);
        return list;
    }
    ...
    public static void set(String key, Object value, long time) {
        redisOperator.set(key, JSON.toJSONString(value), time);
    }
    ...
    private static <T> T get(String key, Class<T> clazz) {
        ...
        return Optional.ofNullable(value).map(v -> JSON.parseObject(v, clazz)).orElse(null);
    }
    ...
    public static <T> List<T> list(String key, Class<T> clazz) {
        String value = redisOperator.get(key);
        return Optional.ofNullable(value).map(v -> JSON.parseArray(v, clazz)).orElse(null);
    }
}
